# PADER Report Generation System — README

A prototype that turns raw ICSR (Individual Case Safety Report) data for
Bisoprolol into a structured, evidence-backed PADER-style report.
...
## Evaluating at scale (1,000 generated reports, not one)

Everything above describes checking one report by hand. That doesn't
scale, so here's how I'd extend it to a batch setting:

**1. Make the grounding check the primary gate, not a nice-to-have.**
At one report, a human reads every sentence. At 1,000 reports, that's not
realistic — so the mechanical grounding check (already deterministic,
already cheap) becomes the first-pass filter. Any report where every
section passes grounding moves to a lighter-touch human spot-check;
anything with even one flagged section goes to a full human review queue.
This inverts the current design's emphasis (human reads everything) into
a triage system (human reads what the machine can't already vouch for).

**2. Track a per-report grounding pass rate as a monitoring metric.**
If 999/1,000 reports pass grounding cleanly and one doesn't, that's a
one-off to investigate. If the pass rate suddenly drops from 99% to 80%
across a batch, that's a signal something upstream changed — a new data
format, a prompt regression after a model update, a packet builder bug —
and it's a metric you'd want on a dashboard, not just a debug tool.

**3. Sample-based human audit, not full-coverage.**
Once the grounding check is trusted (i.e. validated against enough human-
reviewed reports that its false-negative rate is known), a random sample
of N reports per batch — including a forced sample of any report with
unusual source data characteristics — replaces 100% manual review. This
mirrors how audit sampling works in the actual regulatory/QA world.

**4. Regression testing with known-answer cases.**
Keep a small set of report packets with hand-verified correct output
(similar to how the sample PADER PDF was used here as a sanity check for
this exercise). Re-running these after any change to the system prompt,
model version, or packet-building logic should reproduce the same
grounded, correct output — a lightweight CI test rather than trusting
that a prompt change "seems fine."

**5. Cross-report consistency checks.**
At scale, the same underlying case data might feed multiple report types
(PADER, PSUR, etc. — see VERSION_1_DESIGN.md). A case count of 1,024 in
one report and 1,018 in another covering the same period/product is a
structural inconsistency a script can catch automatically, independent
of whether either number individually passed its own grounding check.

**6. What I would NOT do:** add a second LLM as a "judge" to grade the
first LLM's output at scale. As noted above, that's the same class of
model making the same class of mistake, now applied to review — it adds
cost and false confidence without a clear accuracy gain over the
deterministic check plus sampled human audit described above.
...
pader_system/
├── data/
│   └── Bisoprolol_icsr_sample_1068rows.xlsx  (excluded from this zip — see data/README.md)
├── analysis/
